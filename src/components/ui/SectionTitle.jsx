...
const SectionTitle = ({ 
  ...
  const titleParts = typeof title === 'string'
    ? title.trim().split(/\s+/)
    : [title];
  ...
  const renderTitle = () => {
    ...
    // ✅ VARIANT DEFAULT: Kedua fitur berfungsi
    if (variant === 'default') {
      // Jika ada breakCount
      if (breakCount > 0 && breakCount < titleParts.length) {
        ...
        // Render first part dengan highlight jika perlu
        const firstPart = firstPartWords.map((word, idx) => {
          const originalIndex = idx;
          const shouldBeBold = highlightCount > 0 && 
                               originalIndex >= (titleParts.length - highlightCount);
          
          return shouldBeBold ? (
            <span key={idx} className="font-bold italic">{word}</span>
          ) : (
            <span key={idx}>{word}</span>
          );
        }).reduce((prev, curr) => [prev, ' ', curr]);

        // Render last part dengan highlight jika perlu
        const lastPart = lastPartWords.map((word, idx) => {
      ...
      // Jika hanya highlight tanpa break
    ...
    // ✅ VARIANT MINIMAL: Hanya satu fitur
    if (variant === 'minimal') {
      if (highlightCount > 0 && highlightCount < titleParts.length) {
        const normalWords = titleParts.slice(0, -highlightCount).join(' ');
        const highlightedWords = titleParts.slice(-highlightCount).join(' ');
